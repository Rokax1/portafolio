...
const RippleGrid: React.FC<Props> = ({
  ...
  const currentRotationRef = useRef(gridRotation);
  const targetRotationRef = useRef(gridRotation);
  const currentRippleIntensityRef = useRef(rippleIntensity);
  const targetRippleIntensityRef = useRef(rippleIntensity);
  // Suavizado del giro: menor = más lento/suave, mayor = más rápido/directo. Prueba entre 0.01 y 0.08.
  const rotationSmoothing = 0.035;
  // Suavizado del ripple: menor = entra/sale más lento, mayor = responde más inmediato. Prueba entre 0.01 y 0.1.
  const rippleSmoothing = 0.004;
...
void main() {
    ...
    const render = (t: number) => {
      ...
      currentRotationRef.current += (targetRotationRef.current - currentRotationRef.current) * rotationSmoothing;
      uniforms.gridRotation.value = currentRotationRef.current;
      currentRippleIntensityRef.current += (targetRippleIntensityRef.current - currentRippleIntensityRef.current) * rippleSmoothing;
      uniforms.rippleIntensity.value = currentRippleIntensityRef.current;
  ...
  useEffect(() => {
    ...
    targetRotationRef.current = gridRotation;
    targetRippleIntensityRef.current = rippleIntensity;
  }, [
    enableRainbow,
    gridColor,
    rippleIntensity,
